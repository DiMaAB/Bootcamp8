...
// console.log(arr.filter(el=>el.eyeColor==color));
...
//   let  man=arr.filter(el=>el.gender==gender).map(el=>el.name);
...
//   let get=arr.filter(el=>el.isActive==false);
//   return get;
...
//   let user=arr.filter(el=>el.age>min && el.age<max)
//   return user;
// };
...
//    let balan=arr.reduce((acc,bal)=>acc+bal.balance,0);
//   return balan;
// };
...
//   let user=arr.filter(el=>(el.friends).includes(name)).map(el=>el.name)
//   // console.log(user);
//   return user;
// };
...
// Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (поле friends)
  ...
  const getUserNamesSortedByFriendsCount = arr => {
    let name = arr.sort((a, b) => a.friends.length - b.friends.length).map(function(el){
      return el.name;
    })
    return name;
  };
